...
// Assume the 'recipes' array you provided is already declared here
...
function searchHandler() {
    ...
    // 1. Filter recipes based on name, description, or tags
    ...
    // 2. Sort recipes by name (alphabetical) or rating
    const sortedRecipes = filteredRecipes.sort((a, b) => {
        return a.name.localeCompare(b.name);
    });

    // 3. Clear current results and render filtered ones
    renderRecipes(sortedRecipes);
}

// Template for the stars rating
...
// Main template for the recipe card
function recipeTemplate(recipe) {
    ...
    <article class="recipe-card">
        ...
        <div class="recipe-content">
            <p class="recipe-tags">${tagsTemplate(recipe.tags)}</p>
            <h2>${recipe.name}</h2>
